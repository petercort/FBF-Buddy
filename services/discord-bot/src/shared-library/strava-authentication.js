...
import { UsersTable } from '../db-objects.js';
import { getAzureSecretsClient } from './azure-secrets.js';

let stravaClientId;
let stravaClientSecret;

// Call the azureSecrets.js to get the secrets
...
export async function firstTimeAuth(userId, code) {
  try {
    ...
    await UsersTable.update({ 
      ...
      strava_user_id: response.data.athlete.id, 
      strava_connected: true, 
      strava_refresh_token: response.data.refresh_token, 
      strava_expires_at: response.data.expires_at 
    }, { where: { userId: userId } });
...
export async function getStravaAuthentication(userData) {
